#Machine learning as Linear Algebra Applications
################################################################
#
# Chapter 3: Linear Algebra Applications to  Machine Learning
#
################################################################
...
###
#verification of Eq. (9.13)
(1/2)*norm(x1 - x2, type = '2')^2
#[1] 0.5
...
title('(a) 2001 Daily Miami Tmin vs WDF2', 
      cex.main = 0.9, line = 1)
...
plot(x, col = y + 3, pch = 19,
     xlim = c(0, 6), ylim = c(0, 6),
     xlab = bquote(italic(x[1])), 
     ylab = bquote(italic(x[2])),
     cex.lab = 1.5, cex.axis = 1.5,
     main = 'SVM for Three Points Labeled in Two Categories')
axis(2, at = (-2):8, tck = 1, lty = 2, 
     col = "grey", labels = NA)
...
points(xnew, pch = 18, cex = 2)
#for(i in 1:2){
#  text(xnew[i,1] + 0.5, xnew[i,2], bquote(Q[i]),
#       cex = 1.5, col = 6-2*i)
#}
text(0.8, 2.5, bquote(italic(Q[1])), cex = 1.5, col = 4)
text(4.8, 4.0, bquote(italic(Q[2])), cex = 1.5, col = 2)
text(1.2, 0.8, bquote(italic(P[1])), cex = 1.5, col = 4)
text(2.2, 1.2, bquote(italic(P[2])), cex = 1.5, col = 4)
text(3.2, 3.65, bquote(italic(P[3])), cex = 1.5, col = 2)
text(2.2,5.8, "Two blue points and a red point 
are training data for an SVM ", 
     cex = 1.5, col = 4)
text(3.5,4.7, "Two black diamond points 
...
plot(z, 1/(1 + exp(-k*(z - z0))),
     ...
     xlab = 'z', ylab = 'g(z)',
     main = 'Logistic function for k = 3.2, z0 = 1.0',
     cex.lab = 1.3, cex.axis = 1.3)
dev.off()
...
p = 0.8 # assign 50% of data for training
...
table(predicted, test$Species)#prediction vs observation
